...
def rotate_left(num, k):
    bits = 64
    k %= bits
    return ((num << k) | (num >> (bits - k)))&((1<<bits)-1)
...
def read_short(b):
    ...
    if leng >= 4:
        off = (leng & 24) >> (leng >> 3)
        return ((read_u32(b) << 32)&0xFFFFFFFFFFFFFFFF) | read_u32(b[leng-4:]),((read_u32(b[off:]) << 32)&0xFFFFFFFFFFFFFFFF) | read_u32(b[leng-4-off:])
    elif leng > 0:
        # [0] [0] [0] @ len == 1 (0b01)
        # [0] [1] [1] @ len == 2 (0b10)
        # [0] [1] [2] @ len == 3 (0b11)
        # MSB <-> LSB
        return ((b[0]<<48)&0xFFFFFFFFFFFFFFFF)|((b[leng>>1]<<24)&0xFFFFFFFFFFFFFFFF)|b[leng-1],0
    else:
        return 0,0
# convert from lib.rs
def u128_to_u64s(x):
...
def u64s_to_u128(lo,hi):
    return (hi << 64) | (lo&0xFFFFFFFFFFFFFFFF)
...
def _tower_layer_6(state,b,BFAST):
    state[0],state[1] = _frac_6((state[0],state[1]),(read_u64(b[seg(0):]),read_u64(b[seg(1):])),BFAST)
    state[2],state[3] = _frac_6((state[2],state[3]),(read_u64(b[seg(2):]),read_u64(b[seg(3):])),BFAST)
    state[4],state[5] = _frac_6((state[4],state[5]),(read_u64(b[seg(4):]),read_u64(b[seg(5):])),BFAST)
    return state

def _tower_layer_3(state,b,BFAST):
    state[0],state[3] = _frac_3((state[0],state[3]),read_u64(b[seg(0):]),BFAST)
    state[1],state[4] = _frac_3((state[1],state[4]),read_u64(b[seg(1):]),BFAST)
    state[2],state[5] = _frac_3((state[2],state[5]),read_u64(b[seg(2):]),BFAST)
    return state
...
def tower_loong(b,seed,BFAST):
    ...
    if rem >= seg(12):
        ...
        while True:
            state,ring_prev = _tower_layer_12(state,b[off:],ring_prev,BFAST)
            off += seg(12)
            rem -= seg(12)
            if rem < seg(12):
                break
        state[0] ^= ring_prev
    if rem >= seg(6):
        state = _tower_layer_6(state,b[off:],BFAST)
        off += seg(6)
        rem -= seg(6)
    if rem >= seg(3):
        state = _tower_layer_3(state,b[off:],BFAST)
        off += seg(3)
    return _tower_layer_x(_tower_layer_0(state,b[off:],tot_len),tot_len,BFAST)
...
def base_hash_128(b,seed,BFAST):
    if len(b) <= seg(2):
        return epi_short_128(tower_short(b,seed),BFAST)
    else:
        return epi_loong_128(tower_loong(b,seed,BFAST),BFAST)
